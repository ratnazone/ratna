...
namespace Jardalu.Ratna.Web.Upload.Actions
{
    ...
    public class OtherUploadedAction : AbstractMediaUploadedAction
    {
        public override void Saved(HttpRequest request, string actualPath, string vpath)
        {
            ...
            OtherMedia other = new OtherMedia();
            other.Url = vpath;

            try
            {
                other.Owner = AuthenticationUtility.Instance.GetLoggedUser();
                other.Name = Path.GetFileName(actualPath);

                //save the media
                BaseMedia media = MediaStore.Instance.Save(other);
