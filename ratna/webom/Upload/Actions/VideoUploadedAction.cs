...
namespace Jardalu.Ratna.Web.Upload.Actions
{
    ...
    public class VideoUploadedAction : AbstractMediaUploadedAction
    {
        public override void Saved(HttpRequest request, string actualPath, string vpath)
        {
            ...
            if (!SupportedVideoTypes.IsSupported(vpath))
            {
            ...
            else
            {
                ...
                try
                {
                    video.Owner = AuthenticationUtility.Instance.GetLoggedUser();
                    video.Name = Path.GetFileName(actualPath);

                    //save the media
                    BaseMedia media = MediaStore.Instance.Save(video);
